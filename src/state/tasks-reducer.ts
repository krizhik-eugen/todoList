...
export const updateTaskStatusTC = (taskId: string, todolistId: string, status: TaskStatuses) =>
    (dispatch: Dispatch, getState: () => AppRootStateType) => {
        ...
        const clickedTask = tasksForClickedTodo.find((el) => {
            return el.id === taskId
        })

        if (clickedTask) {
            let model = {...clickedTask, status}
...
export const changeTaskTitleTC = (taskId: string, todolistId: string, title: string) =>
    (dispatch: Dispatch, getState: () => AppRootStateType) => {
        ...
        const stateApp = getState();
        const allTasks = stateApp.tasks;
        const taskWithClickedTitle = allTasks[todolistId]
        const taskForChanging = taskWithClickedTitle.find((el) => {
            return el.id === taskId
        })
        if (taskForChanging) {
            let model = {...taskForChanging, title}
            todolistsAPI.updateTask(todolistId, taskId, model)
                .then(() => {
