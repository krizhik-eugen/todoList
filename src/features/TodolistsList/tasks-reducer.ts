...
export const changeTasksEntityStatusAC = (taskId: string, todolistId: string, entityStatus: RequestStatusType) => ({
    type: 'CHANGE-TASKS-ENTITY-STATUS',
    taskId,
    todolistId,
    entityStatus
} as const)
...
export const removeTaskTC = (taskId: string, todolistId: string) => (dispatch: Dispatch<ActionsType>) => {
    dispatch(setAppStatusAC('loading'))
    dispatch(changeTasksEntityStatusAC(taskId, todolistId,'loading'))
        ...
        todolistsAPI.updateTask(todolistId, taskId, apiModel)
            .then(res => {
                if (res.data.resultCode === 0) {
                    ...
                    dispatch(action)
                    dispatch(setAppStatusAC('succeeded'))
                    dispatch(changeTasksEntityStatusAC(taskId, todolistId,'succeeded'))
                } else {
